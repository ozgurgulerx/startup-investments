...
#
# Usage:
#   runner.sh global-analysis 960 /opt/buildatlas/startup-analysis/infrastructure/vm-cron/jobs/global-analysis.sh [PERIOD] [CONCURRENT]
#
...
# Run incremental analysis (delta detection, no viral)
# Uses AnalysisStore which writes to analysis_store/base_analyses/
echo ""
...
cd "$REPO_DIR"
git add "apps/web/data/$PERIOD/output/analysis_store/" || true
git add "apps/web/data/$PERIOD/output/raw_content/" || true
if ! git diff --staged --quiet; then
    git commit -m "Global analysis: enrich $PERIOD startups with crawl + LLM analysis [skip ci]"
    git push
