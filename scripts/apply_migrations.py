#!/usr/bin/env python3
"""
Shared database migration runner (VM + AKS).

This is the single source of truth for migration-set -> migration files mapping.
...
def _repo_root() -> Path:
    # scripts/apply_migrations.py -> repo root
...
# IMPORTANT: keep this mapping in sync with VM/cron expectations.
...
def _files_for_set(migration_set: str) -> list[str]:
    if migration_set == "all":
        return sorted({f for files in SETS.values() for f in files})
    return SETS.get(migration_set, [])
...
async def _run(db_url: str, migration_set: str) -> int:
    try:
        import asyncpg  # type: ignore
    except Exception as e:
        sys.stderr.write(f"ERROR: asyncpg import failed: {e}\n")
        return 2

    files = _files_for_set(migration_set)
    if not files:
        sys.stderr.write(f"ERROR: Unknown/empty migration set: {migration_set}\n")
        return 2

    base = _repo_root() / "database" / "migrations"
    failed: list[str] = []

    conn = await asyncpg.connect(db_url)
    acquired = False
    try:
        # Serialize migrations across VM cron + AKS CronJobs to avoid concurrent DDL deadlocks.
        lock_wait_sec = max(
            5,
            int((os.environ.get("BUILDATLAS_MIGRATIONS_LOCK_WAIT_SECONDS") or "300").strip() or "300"),
        )
        lock_id = 747191  # Stable BuildAtlas advisory lock id (int8).
        start = time.monotonic()
        while True:
            row = await conn.fetchrow("SELECT pg_try_advisory_lock($1) AS ok", lock_id)
            acquired = bool(row["ok"]) if row and ("ok" in row) else False
            if acquired:
                break
            if time.monotonic() - start >= lock_wait_sec:
                print("SKIP: migrations advisory lock busy; another job may be applying migrations.")
                return 0
        ...
        for filename in files:
            ...
            try:
                ...
                print(f"  FAILED: {filename}: read failed: {e}")
                failed.append(filename)
                continue

            # DDL can still deadlock (even serialized) if other tools apply migrations without advisory lock.
            max_attempts = 5
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if _is_transient_ddl_error(e) and attempt < max_attempts:
                        delay = (0.5 * (2 ** (attempt - 1))) + (random.random() * 0.25)
                        print(
                            f"  RETRY: {filename} (attempt {attempt}/{max_attempts}) after {delay:.2f}s: {e}"
                        )
        ...
        if acquired:
            try:
                await conn.execute("SELECT pg_advisory_unlock($1)", 747191)
            except Exception:
                pass
        await conn.close()
...
def main() -> int:
    migration_set = (sys.argv[1] if len(sys.argv) > 1 else "all").strip()
    print(f"Applying migrations (set: {migration_set})...")

    try:
        db_url = _env_required("DATABASE_URL")
    except Exception as e:
        sys.stderr.write(f"ERROR: {e}\n")
        return 2

    return asyncio.run(_run(db_url, migration_set))
