...
def test_resolve_edition_date_parses_explicit_string_to_date():
    ...
    resolved = asyncio.run(sender._resolve_edition_date(FakeConn(date(2026, 2, 7)), "2026-02-07", region="global"))
...
def test_resolve_edition_date_accepts_db_date_type():
    ...
    resolved = asyncio.run(sender._resolve_edition_date(FakeConn(date(2026, 2, 7)), None, region="global"))
...
def test_resolve_edition_date_accepts_db_string_type():
    ...
    resolved = asyncio.run(sender._resolve_edition_date(FakeConn("2026-02-07"), None, region="global"))
    assert resolved == date(2026, 2, 7)
