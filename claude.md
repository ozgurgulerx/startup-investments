...
These values are HARDCODED and must be updated in the code if they change:
...
| Redis Cache | `aistartupstr-redis-cache` | `aistartupstr` | **RUNNING** | API response caching (Standard, SSL port 6380) |
...
## Backend Availability (KEEP RUNNING AT ALL TIMES)

**The AKS cluster and API backend MUST remain running.** Stopping AKS causes the API to go down (504 Gateway Timeout via Front Door), and the frontend degrades to slow file-based data loading.

### NEVER Stop These Services

```bash
# FORBIDDEN - These commands will take down the backend:
...
```bash
# 1. Check all services in one go
echo "=== Frontend ===" && \
