...
function statusNotice(params?: { confirmed?: string; unsubscribed?: string }) {
  const confirmed = params?.confirmed;
  const unsubscribed = params?.unsubscribed;
  let text = '';
  let tone: 'success' | 'warn' = 'success';

  if (unsubscribed === '1') {
    text = 'Unsubscribed. You will no longer receive digest emails.';
    tone = 'success';
  } else if (unsubscribed === '0') {
    text = 'Unsubscribe link was invalid or already used.';
    tone = 'warn';
  } else if (confirmed === '1') {
    text = 'Subscription confirmed. You are now subscribed to the Turkey Signal Feed.';
    tone = 'success';
  } else if (confirmed === 'already') {
    text = 'Already confirmed. You are subscribed to the Turkey Signal Feed.';
    tone = 'success';
  } else if (confirmed === 'expired') {
    text = 'Confirmation link expired or invalid. Subscribe again to receive a new link.';
    tone = 'warn';
  }
  ...
  return (
    <section className={`mt-6 rounded-xl border px-4 py-3 text-sm ${
      tone === 'success'
