...
export default async function TurkeyWeeklyBriefPage() {
  const [brief, archive] = await Promise.all([
    withTimeout(getPeriodicBrief({ periodType: 'weekly', region: 'turkey' }), 5000).catch(() => null),
      ...
      <div className="flex h-screen flex-col bg-background">
        ...
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-md rounded-2xl border border-border/40 bg-card/60 p-10 text-center">
            <p className="label-xs text-accent-info">Turkiye Haftalik Bulten</p>
            <h1 className="mt-3 text-3xl font-light tracking-tight text-foreground">Haftalik bulten henuz yok</h1>
            <p className="mt-4 text-sm text-muted-foreground">
              Haftalik bultenler her pazartesi uretilir. Kisa sure sonra tekrar kontrol edin.
            </p>
          </div>
        </div>
      </div>
