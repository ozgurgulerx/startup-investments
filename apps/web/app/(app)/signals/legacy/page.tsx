...
async function LegacySignalsContent({ region }: { region?: string }) {
  const normalizedRegion = normalizeDatasetRegion(region);
  const availablePeriods = await getAvailablePeriods(normalizedRegion);
  const latestPeriod = availablePeriods[0]?.period || FALLBACK_PERIOD;

  const [stats, startups] = await Promise.all([
    getMonthlyStats(latestPeriod, normalizedRegion),
    getStartups(latestPeriod, normalizedRegion),
  ]);
...
export default async function SignalsLegacyPage({
  searchParams,
}: {
  ...
  return (
    <Suspense fallback={<LegacySignalsLoading />}>
      <LegacySignalsContent region={region} />
    </Suspense>
  );
}
