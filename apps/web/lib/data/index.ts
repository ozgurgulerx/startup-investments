...
export async function getFilterOptions(
  ...
  if (isAPIConfigured()) {
    try {
      ...
      // If the API is ahead/behind compared to deployed files (or DB isn't yet synced for the region),
      // degrade to file-based computation so the UI stays non-empty.
      if (!isEmpty) {
        return response;
      }
    } catch (error) {
